...
var imagesPath = "extern/rte/images/";
var includesPath = "extern/rte/";
var cssFile = "extern/rte/rte.css";
...
function writeRichText(rte, html, width, height, buttons, readOnly) {
	if (isRichText) {
		...
		if (buttons == true) {
			...
			document.writeln('		<td>');
			document.writeln('			<select border="0" class="rteSelect" id="formatblock_' + rte + '" onchange="selectFont(\'' + rte + '\', this.id);">');
...
// 			}
	...
	//		document.writeln('		<td><img class="rteImage" src="' + imagesPath + 'redo.gif" width="25" height="24" alt="Redo" title="Redo" onClick="rteCommand(\'' + rte + '\', \'redo\')"></td>');
		...
		if (!isIE) width = parseInt(width) - 2;
		else width = parseInt(width) - 6;

		document.writeln('<iframe frameborder="0" class="rteIFrame" id="' + rte + '" name="' + rte + '" width="' + width + 'px" height="' + height + 'px" src="' + includesPath + 'blank.htm"></iframe>');
		if (!readOnly) document.writeln('<div style="font-size: 1.1em; position: relative; top: 3px;"><input style="position: relative; top: 2px;" type="checkbox" id="chkSrc' + rte + '" onclick="toggleHTMLSrc(\'' + rte + '\');" />&nbsp;View Source</div>');
		document.writeln('<iframe frameborder="0" class="rtePallette" width="152" height="106" id="cp' + rte + '" src="' + includesPath + 'palette.htm" marginwidth="0" marginheight="0" scrolling="no" style="visibility:hidden; position: absolute;"></iframe>');
...
function updateRTE(rte) {
	...
	if (isRichText && !readOnly) {
		...
		oHdnMessage.value = oHdnMessage.value.replace(/<br([^>]*)\s*>/gi, '<br$1 />');
		oHdnMessage.value = oHdnMessage.value.replace(/<img([^>]*)\s*>/gi, '<img$1 />');
		oHdnMessage.value = oHdnMessage.value.replace(/<hr([^>]*)\s*>/gi, '<hr$1 />');
		oHdnMessage.value = oHdnMessage.value.replace(/\n/gi, ' ');
		oHdnMessage.value = oHdnMessage.value.replace(/\r/gi, ' ');
		oHdnMessage.value = oHdnMessage.value.replace(/\r\n/gi, ' ');
	}
}
